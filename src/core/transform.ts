...
// const md5 = (str: string) => crypto.createHash('md5').update(str).digest('hex').substring(0, 6)
...
function transform(code: string, id: string, options = { serializePath: true }) {
  ...
  let result
  // id = options.serializePath ? md5(id) : id
  switch (extname) {
    case '.vue':
      result = generate(id, code, compiler.compileVue(code))
      break
    case '.jsx':
    case '.tsx':
    case '.svelte':
